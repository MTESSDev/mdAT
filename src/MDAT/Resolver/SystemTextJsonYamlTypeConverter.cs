...
public sealed class SystemTextJsonYamlTypeConverter : IYamlTypeConverter
{
    ...
    private bool UseInvariantCulture { get; }

    /// <summary>
    /// Allows YamlDotNet to de/serialize System.Text.Json objects
    /// </summary>
    /// <param name="sortKeysAlphabetically">sorts keys alphabetically when Serializing</param>
    /// <param name="useInvariantCulture">Force using invariant culture to respect Json values</param>
    public SystemTextJsonYamlTypeConverter(bool sortKeysAlphabetically = false, bool useInvariantCulture = false)
    {
    ...
    private object ReadJsonValue(IParser parser)
    {
        if (parser.TryConsume<Scalar>(out var scalar))
        {
            if (scalar.Style == ScalarStyle.Plain)
            {
                ...
                else if (float.TryParse(scalar.Value, NumberStyles.Float, UseInvariantCulture ? CultureInfo.InvariantCulture : null, out var f))
                {
